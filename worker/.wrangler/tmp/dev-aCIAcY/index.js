...
// .wrangler/tmp/bundle-F22jOE/checked-fetch.js
...
// .wrangler/tmp/bundle-F22jOE/strip-cf-connecting-ip-header.js
...
__export(schema_exports, {
  ...
  agreementsRelations: () => agreementsRelations,
  auditLog: () => auditLog,
...
var agreements = sqliteTable("agreements", {
  id: integer("id").primaryKey({ autoIncrement: true }),
...
var productTypesRelations = relations(productTypes, ({ one, many }) => ({
...
var agreementsRelations = relations(agreements, ({ one }) => ({
...
async function authMiddleware(c, next) {
  ...
  if (cfAccessEmail) {
    ...
    if (!user) {
      ...
      const displayName = emailPrefix.split(/[._-]/).map((part) => part.charAt(0).toUpperCase() + part.slice(1).toLowerCase()).join(" ");
      ...
      const emailPrefix = cfAccessEmail.split("@")[0];
      const needsDisplayNameUpdate = !user.displayName || user.displayName === emailPrefix || user.displayName === cfAccessEmail;
      const updates = {
        lastLogin: (/* @__PURE__ */ new Date()).toISOString()
      };
      if (needsDisplayNameUpdate) {
        updates.displayName = emailPrefix.split(/[._-]/).map((part) => part.charAt(0).toUpperCase() + part.slice(1).toLowerCase()).join(" ");
      }
...
app2.post("/", requireRole("admin", "editor"), async (c) => {
  const db = createDb(c.env.DB);
  const body = await c.req.json();
...
app2.put("/:id", requireRole("admin", "editor"), async (c) => {
  const db = createDb(c.env.DB);
  const id = parseInt(c.req.param("id"));
  const body = await c.req.json();
  const [result] = await db.update(productTypes).set({
    ...body,
    updatedAt: (/* @__PURE__ */ new Date()).toISOString()
  }).where(eq(productTypes.id, id)).returning();
  if (!result) {
    return c.json({ error: "Product type not found" }, 404);
  }
  return c.json(result);
...
app2.delete("/:id", requireRole("admin"), async (c) => {
  const db = createDb(c.env.DB);
  const id = parseInt(c.req.param("id"));
  const [result] = await db.delete(productTypes).where(eq(productTypes.id, id)).returning();
  if (!result) {
    return c.json({ error: "Product type not found" }, 404);
  }
  return c.json({ success: true });
});
var product_types_default = app2;
...
app3.get("/", async (c) => {
  ...
  if (search) {
    whereClause = and(
      activeOnly ? eq(customers.isActive, true) : void 0,
      or(
  ...
  const result = await db.query.customers.findMany({
    ...
    orderBy: (customers3, { asc: asc2 }) => [asc2(customers3.name)]
  });
...
app3.post("/", requireRole("admin", "editor"), async (c) => {
  const db = createDb(c.env.DB);
  const body = await c.req.json();
  if (!body.name) {
    return c.json({ error: "Name is required" }, 400);
...
app3.put("/:id", requireRole("admin", "editor"), async (c) => {
  const db = createDb(c.env.DB);
  const id = parseInt(c.req.param("id"));
  const body = await c.req.json();
...
app3.delete("/:id", requireRole("admin"), async (c) => {
  const db = createDb(c.env.DB);
  const id = parseInt(c.req.param("id"));
  const [result] = await db.update(customers).set({
    isActive: false,
...
app4.get("/", async (c) => {
  ...
  const result = await db.query.agreements.findMany({
    where: activeOnly ? eq(agreements.isActive, true) : void 0,
    with: {
      customer: true,
      productType: {
        with: {
    ...
    orderBy: (agreements2, { asc: asc2 }) => [asc2(agreements2.nextBillingDate)]
  });
  let filtered = result;
  if (customerId) {
...
app4.post("/", requireRole("admin", "editor"), async (c) => {
  const db = createDb(c.env.DB);
  const body = await c.req.json();
...
app4.put("/:id", requireRole("admin", "editor"), async (c) => {
  const db = createDb(c.env.DB);
  const id = parseInt(c.req.param("id"));
  const body = await c.req.json();
  const [result] = await db.update(agreements).set({
    ...body,
    updatedAt: (/* @__PURE__ */ new Date()).toISOString()
  }).where(eq(agreements.id, id)).returning();
  if (!result) {
    return c.json({ error: "Agreement not found" }, 404);
  }
  return c.json(result);
});
app4.delete("/:id", requireRole("admin"), async (c) => {
  const db = createDb(c.env.DB);
  const id = parseInt(c.req.param("id"));
  const [result] = await db.update(agreements).set({
    isActive: false,
...
app4.post("/:id/bill", requireRole("admin", "editor"), async (c) => {
  const db = createDb(c.env.DB);
  const id = parseInt(c.req.param("id"));
...
var agreements_default = app4;

// src/routes/reports.ts
var app5 = new Hono2();
app5.get("/revenue", async (c) => {
  const db = createDb(c.env.DB);
...
app5.get("/dashboard", async (c) => {
  ...
  const today = /* @__PURE__ */ new Date();
  const thirtyDaysAhead = /* @__PURE__ */ new Date();
...
app6.get("/", async (c) => {
  ...
  const result = await db.query.users.findMany({
    orderBy: (users2, { asc: asc2 }) => [asc2(users2.displayName)]
  });
  return c.json(result);
});
app6.get("/me", async (c) => {
...
app6.put("/:id/role", requireRole("admin"), async (c) => {
  const db = createDb(c.env.DB);
  const id = parseInt(c.req.param("id"));
  const body = await c.req.json();
...
app6.delete("/:id", requireRole("admin"), async (c) => {
  const db = createDb(c.env.DB);
  const id = parseInt(c.req.param("id"));
...
app7.route("/api/product-types", product_types_default);
app7.route("/api/customers", customers_default);
app7.route("/api/agreements", agreements_default);
app7.route("/api/reports", reports_default);
app7.route("/api/users", users_default);
app7.notFound((c) => {
...
// .wrangler/tmp/bundle-F22jOE/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-F22jOE/middleware-loader.entry.ts
